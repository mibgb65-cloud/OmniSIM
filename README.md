...
| 范围 | 能力 |
| --- | --- |
| 续费管理 | 按日期排序、逾期/今日到期/即将到期状态、固定天数周期、每月固定日期及手动日期 |
...
| 成本 | 显示 30/365 天实际支出与续费价格变化，并使用欧洲央行每日及欧盟委员会月度汇率估算未来成本 |
| 数据 | Room 本地数据库、DataStore 设置、版本化 JSON 备份与事务性恢复 |
| 更新 | 冷启动静默检查最新正式版本，在设置中可手动检查、查看内容并下载签名 APK |
...
| 网络访问 | 获取欧洲央行、欧盟委员会公开汇率和 GitHub 正式版本信息；用户确认后，应用内下载并校验正式 APK |
...
| `INTERNET` | 获取公开汇率、检查 GitHub 正式版本，并在用户确认后下载 APK 和校验文件 |
| `REQUEST_INSTALL_PACKAGES` | 将校验通过的正式 APK 交给 Android 安装器；系统仍会要求用户确认 |

续费网址通过外部浏览器打开；确认后的正式 APK 在应用内下载并校验，再交给 Android 系统安装器；版本比较与更新内容在应用内显示。
...
flowchart TD
    ...
    RATE --> ECB[ECB 每日参考汇率]
    RATE --> EUFX[欧盟委员会 InforEuro 月度汇率]
```
...
单元测试覆盖日期计算、状态优先级、提醒匹配与去重、汇率解析、成本换算、更新包
筛选、备份校验、CSV 导出和续费历史筛选。
...
并重新调度。后台检查延迟跨过提醒点时，只补发最近一条尚未发送的提醒，避免集中弹出。
“设置 → 续费与通知”会显示最近扫描及发送数量，并提供通知和后台运行设置入口。
